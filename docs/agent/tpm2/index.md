...
- `embedded_linux_audit tpm2` no longer depends on installed `tpm2-tools` binaries.
...
```bash
./embedded_linux_audit tpm2 list-commands
./embedded_linux_audit tpm2 getcap properties-fixed
./embedded_linux_audit tpm2 pcrread sha256:0,1,2
./embedded_linux_audit tpm2 nvreadpublic 0x1500016
./embedded_linux_audit tpm2 createprimary -C o -g sha256 -G rsa -c primary.ctx
```
