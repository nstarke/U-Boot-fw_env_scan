...
run_accept_case "linux execute-command valid global --output-http with unreachable endpoint" "$BIN" --output-http http://127.0.0.1:1 linux execute-command "echo hello"
run_accept_case "linux execute-command repeated global --output-http" "$BIN" --output-http http://127.0.0.1:1 --output-http https://127.0.0.1:1 linux execute-command "echo hello"
...
rc=$?
if [ "$rc" -eq 0 ] && ! grep -q 'log agent_timestamp=' "$log"; then
    echo "[PASS] linux execute-command suppresses lifecycle logs by default"
    PASS_COUNT="$(expr "$PASS_COUNT" + 1)"
else
    echo "[FAIL] linux execute-command suppresses lifecycle logs by default (rc=$rc)"
    print_file_head_scrubbed "$log" 120
    FAIL_COUNT="$(expr "$FAIL_COUNT" + 1)"
fi
rm -f "$log"

if [ "${ELA_TEST_ISA:-}" = "arm32-be" ]; then
    echo "[SKIP] linux execute-command emits lifecycle logs when ELA_DEBUG=1 (arm32-be: lifecycle log segfaults under older QEMU user-mode emulation)"
else
    log="$(mktemp /tmp/test_execute_command_lifecycle_debug.XXXXXX)"
    ...
    if [ "$rc" -eq 0 ] && \
       ...
       grep -Eq 'log agent_timestamp=[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z phase=complete command=linux execute-command echo hello rc=0' "$log"; then
        ...
        echo "[FAIL] linux execute-command emits lifecycle logs when ELA_DEBUG=1 (rc=$rc)"
        print_file_head_scrubbed "$log" 120
        FAIL_COUNT="$(expr "$FAIL_COUNT" + 1)"
    fi
    rm -f "$log"
